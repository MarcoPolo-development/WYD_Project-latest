#include "wyd_Rupture.h"


uint8_t encodingKeys[512] = {
    };

bool Wyd_Rupture::Decode(uint8_t *buffer, int32_t offset, int32_t length)
{
    uint8_t keyword = *((uint8_t *)(buffer + offset + 2));
    uint8_t checksum = *((uint8_t *)(buffer + offset + 3));
    uint8_t checksumEncoded = 0, checksumDecoded = 0;
    uint8_t keyIncrement = encodingKeys[keyword * 2];

    int32_t pos = keyIncrement;

    for (int32_t i = 4; i < length; i++, pos++)
    {
        int32_t rest = pos % 256;

        checksumEncoded += buffer[offset + i];

        uint8_t keyResult = encodingKeys[rest * 2 + 1];

        switch (i & 3)
        {
        case 0:
            buffer[offset + i] = buffer[offset + i] - (keyResult << 1);
            break;

        case 1:
            buffer[offset + i] = buffer[offset + i] + (keyResult >> 3);
            break;

        case 2:
            buffer[offset + i] = buffer[offset + i] - (keyResult << 2);
            break;

        case 3:
            buffer[offset + i] = buffer[offset + i] + (keyResult >> 5);
            break;
        }
        checksumDecoded += buffer[offset + i];
    }

    uint8_t currentChecksum = checksumEncoded - checksumDecoded;

    return currentChecksum == checksum;
}

bool Wyd_Rupture::Encode(uint8_t *buffer, int32_t offset, int32_t length)
{
    uint8_t keyword = rand() % 256;
    uint8_t keyIncrement = encodingKeys[keyword * 2];

    uint8_t checksumEncoded = 0, checksumDecoded = 0;

    int32_t pos = keyIncrement;

    for (int32_t i = 4; i < length; i++, pos++) {

        checksumDecoded += buffer[offset + i];

        int32_t rest = pos % 256;

        uint8_t keyResult = encodingKeys[rest * 2 + 1];

        switch (i & 3) {
            case 0:
                buffer[offset + i] = buffer[offset + i] + (keyResult << 1);
                break;
            case 1:
                buffer[offset + i] = buffer[offset + i] - (keyResult >> 3);
                break;
            case 2:
                buffer[offset + i] = buffer[offset + i] + (keyResult << 2);
                break;
            case 3:
                buffer[offset + i] = buffer[offset + i] - (keyResult >> 5);
                break;
        }

        checksumEncoded += buffer[offset + i];
    }

    uint8_t checksum = checksumEncoded - checksumDecoded;

    buffer[offset + 2] = keyword;
    buffer[offset + 3] = checksum;

    return true;
}


